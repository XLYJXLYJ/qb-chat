...
export default {
    data () {
        return {
            ...
            humanBaseUrl:'http://test.customer.qb-tech.net',//人工url  http://192.168.1.66:8009  http://test.customer.qb-tech.net
    ...
    created(){
        ...
        if(window.location.href.indexOf("test") > 0 || window.location.href.indexOf("127.0.0.1") > 0 || window.location.href.indexOf("localhost") > 0 || window.location.href.indexOf("192.168.1.16") > 0) {
            console.log('进入测试环境')
            this_.humanBaseUrl = "http://192.168.1.66:8009"; // http://192.168.1.66:8009 http://test.customer.qb-tech.net
            this_.appkey = "82hegw5u8xncx";//82hegw5u8ytdx正式  8w7jv4qb829cy测试 82hegw5u8xncx重构测试
        }else{
            console.log('进入正式环境')
            this_.humanBaseUrl = "http://192.168.1.66:8009";
            this_.appkey = "82hegw5u8xncx";
        }
        this_.controlYongyunMessage = 1
        this_.msgs = window.msgs || []
        // localStorage.setItem('uuid', window.dialog_id);
        let userData = Qs.stringify({
            key: window.key,
            dialog_id:localStorage.getItem('uuid') || ''
        });
            ...
            onChanged: function (status) {
                ...
                switch (status) {
                    case RongIMLib.ConnectionStatus.CONNECTED:
                      ...
                      // let this_ = this
                      console.log('请求/chat/priority获得的数据')
                      Vue.axios.post('http://test.open.qb-tech.net/api/v1/chat/priority',datakey)
                        .then(result => {
                          console.log('请求/chat/priority获得的数据')
                          console.log(result)
                          let value = result.data
              ...
              .then(result => {
                ...
                if(result.msg.data instanceof Array){
                      ...
                      if(i==result.msg.data.length-1){
                          if(result.hasOwnProperty('target')){
                            data = {
                                ...
                                whether:[], whetherposition:0, target:result.target, textfold:result.msg.data[result.msg.data.length-1].length>250?true:false, istextfold:result.msg.data[result.msg.data.length-1].length>250?1:0,
                              }
                          }else{
                            data = {
                                msg: result.msg.data[result.msg.data.length-1], src: '', self: false, imgData:false, zan:1, overzan:0, robotZan:1, select:[], selectposition:0,
                                whether:[], whetherposition:0, target:'', textfold:result.msg.data[result.msg.data.length-1].length>250?true:false, istextfold:result.msg.data[result.msg.data.length-1].length>250?1:0,
                              }
                          }
                        ...
                        data = {
                          ...
                          whetherposition:0, target:result.target, textfold:result.msg.data[i].length>250?true:false, istextfold:result.msg.data[i].length>250?1:0,
                        }
                      }
                      // }
                      this_.msgs.push(data);
                  ...
                  data = {
                    ...
                    whether:[], whetherposition:0, target:result.target, textfold:result.msg.data.length>250?true:false, istextfold:result.msg.data.length>250?1:0,
                  }
                  // }
                  this_.msgs.push(data);
                  _data.push(data);
